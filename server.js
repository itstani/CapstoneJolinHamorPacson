...
app.get('/api/notifications', async (req, res) => {
  try {
    ...
    const recentlyApprovedEvent = await eventsCollection.findOne({
      userId: userId,
      status: 'approved',
      approvedAt: { $gt: new Date(Date.now() - 24 * 60 * 60 * 1000) } // Approved in the last 24 hours
    });

    const recentlyRespondedConcern = await concernsCollection.findOne({
      userId: userId,
      status: 'responded',
      respondedAt: { $gt: new Date(Date.now() - 24 * 60 * 60 * 1000) } // Responded in the last 24 hours
    });

    res.json({
      eventApproved: !!recentlyApprovedEvent,
      concernResponded: !!recentlyRespondedConcern
    });
  } catch (error) {
